...
BigInteger BigInteger::Add(const BigInteger &num1, const BigInteger &num2, int32_t sign) {
  ...
  while (temp.data_.back() == 0 && temp.data_.size() > 1)
    temp.data_.pop_back();
  if (temp.data_.size() == 1 && temp.data_.back() == 0)
    temp.sign_ = 1;
  return temp;
}
...
std::pair<BigInteger, BigInteger> BigInteger::DivMod(const BigInteger &numerator, const BigInteger &denominator) {
  if (denominator >= MAX || denominator <= -MAX)
    return BigDivMod(numerator, (BigInteger) denominator);
...
bool BigInteger::IsLess(const BigInteger &num) const {
  ...
  for (size_t i = size - 1; i >= 0; --i) {
    if (vector_at(this->data_, i) < vector_at(num.data_, i))
      return true;
    if (vector_at(this->data_, i) > vector_at(num.data_, i))
      return false;
  }
...
BigInteger BigInteger::operator*(const BigInteger &num) {
  ...
  for (size_t i = 0; i < data_.size(); ++i) {
    for (size_t j = 0; j < num.data_.size(); ++j) {
      ...
      if (temp.data_.size() <= i + j)
        temp.data_.resize(i + j + 1);
      temp.data_[i + j] = now % MAX;
      now /= (uint64_t) MAX;
    }
    if (now > 0) {
      if (temp.data_.size() <= i + num.data_.size())
        temp.data_.resize(i + num.data_.size() + 1);
...
std::string BigInteger::toString() const {
  ...
  for (auto i = data_.rbegin(); i != data_.rend(); ++i) {
    ...
    if (f)
      s.append(std::string(9 - now.size(), '0'));
    s.append(now);
    f = true;
  }
  if (s.empty())
